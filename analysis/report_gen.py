...
def build_pdf(rs, mev, mm, sp, vr, st, n=20000):
    ...
    tbl = [['Метрика', 'Значение', 'Оценка'],
        ...
        ['CPU Self-play (новые, v320)', '90% vs rand, P1=55%', 'Лёгкий P1+'],
        ['GPU Self-play (146 итер)', 'Loss 0.21→0.10', '840K параметров'],
    ...
    comparison_data = [
        ...
        ['MCTS vs Random', '90%', '90%', 'Без разницы'],
        ['Self-play P1:P2', '50:50 (v500)', '55:45 (v320)', 'P1 +5%'],
    ]
    ...
    s.append(Paragraph(
        "Проведено 320 итераций CPU self-play на новых правилах (с нуля, 64 нейрона, 25 партий/итер). "
        "Loss: 1.14 → 0.82 (min), стабильно 0.85-0.94 — снижается медленнее чем на старых правилах "
    ...
    s.append(Paragraph(
        ...
        "Vectorized self-play: 15 параллельных партий, 4 кандидата → 1 GPU батч. "
        "Loss: 0.21 → 0.098 (min на v15), затем рост до 0.15 — переобучение на буфере. "
        "WR vs Random: v1=73%, v25=70%, v50=67%, v75=57%, v100=73%, v125=63%. "
        "Нестабильный — нейросеть выбирала лучший из 4 рандомных ходов (слабый exploration). "
        "Буфер достиг 200,000 сэмплов. Время итерации: 5с (начало) → 38с (конец, большой буфер).", body))

    gpu_data = [
        ['Версия', 'Loss', 'WR vs Random', 'Буфер', 'Время/итер'],
        ['v1', '0.210', '73%', '22K', '5с'],
        ['v10', '0.108', '—', '36K', '8с'],
        ['v25', '0.100', '70%', '59K', '12с'],
        ['v50', '0.111', '67%', '98K', '20с'],
        ['v75', '0.124', '57%', '137K', '26с'],
        ['v100', '0.140', '73%', '175K', '35с'],
        ['v125', '0.151', '63%', '200K', '39с'],
        ['v146', '0.150', '—', '200K', '39с'],
    ]
    gt = Table(gpu_data, colWidths=[22*MM, 22*MM, 30*MM, 22*MM, 25*MM])
    gt.setStyle(tbl_style())
    s.append(gt); s.append(Spacer(1, 4*MM))

    s.append(Paragraph(
        "<b>Наблюдения:</b> GPU-сеть (840K параметров) учится значительно быстрее CPU-сети (8K параметров): "
        "loss 0.21→0.10 за 15 итераций (CPU: 1.10→0.85 за 80 итераций). "
        "Однако WR нестабильный — для ResNet нужен более сильный self-play (MCTS-guided или больше кандидатов). "
        "GPU тренер подготовлен для дальнейшего обучения на новых правилах.", body))
    ...
    for i, c in enumerate([
        ...
        "<b>Новая механика (перенос чужих):</b> ~7 закрытий за оппонента за партию, "
        "партии короче на 4 хода. Добавляет глубину — жертвенные ходы.",
