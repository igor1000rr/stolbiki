...
addPost('v44-audit', 'v4.4: Полный архитектурный аудит — 50 коммитов, 150 тестов', 'v4.4: Full architecture audit — 50 commits, 150 tests',
  'Масштабный аудит: 50 коммитов, 70+ файлов, 3000+ строк.\n\n**Безопасность (8):** Admin ENV, JWT 7d+refresh, replay limits, error reporting, graceful shutdown\n**Производительность (10):** GPU weights –60%, 6 SQL индексов, applySettings batch, collector кеш, StorageEvent вместо polling\n**Архитектура (12):** GameContext v2 (25 events→0), schema migrations, Smart SW, rooms cleanup\n**Фичи:** Аккаунт (пароль/экспорт/удаление), друзья (decline/remove), WS reconnect UI, matchmaking rating, online timers sync\n**Тесты:** 41→150 (+109). dist: 14MB→5.8MB (–59%). **Рефактор:** Game.jsx –234, Profile.jsx –415 (→979). 10 extracted hooks/компонентов\n\n**Безопасность:**\n- Admin через ENV переменную ADMIN_USERNAMES вместо хардкода имени\n- Trust proxy для корректного IP за Nginx (rate limiting)\n- Валидация паролей синхронизирована (min 6 символов)\n- Лимит размера training data (100KB)\n- Graceful shutdown: уведомление игроков, закрытие DB\n\n**Производительность:**\n- GPU weights: 8.1MB JSON → 3.2MB binary (–60%), lazy load только для Hard+\n- 6 новых SQL индексов (rating_history, achievements, training, seasons, daily, puzzles)\n- Analytics endpoint: LIMIT 2000 вместо загрузки всех игр\n- last_seen throttle: обновление раз в 5 мин вместо каждого запроса\n- N+1 запрос профиля → единый SQL с RANK() OVER\n\n**Архитектура:**\n- React Context (GameContext) вместо 5 window.* глобалов\n- PM2: log rotation, graceful shutdown, логи в файлы\n- CI/CD: деплой server/ целиком вместо списка файлов\n- Утилита конвертации весов в binary формат\n\n**Новое в v4.4.26+:**\n- dist: 14MB→5.8MB (–59%) — убран JSON fallback weights\n- PWA Install Prompt: кнопка «Установить»\n- ProfileAnalytics, ProfileAccount, ProfileFriends — extracted из Profile.jsx\n- BreadcrumbList + FAQPage JSON-LD, preconnect, noscript\n- DB maintenance, SQLite pragmas, anti-cheat, 404 handler\n- CORS preflight cache, rate limiter cleanup, X-Response-Time\n- Online lobby: live games indicator',
  'Massive audit: 50 commits, 70+ files, 3000+ lines.\n\n**Security (8):** Admin ENV, JWT 7d+refresh, replay limits, error reporting, graceful shutdown\n**Performance (10):** GPU weights –60%, 6 SQL indexes, applySettings batch, collector cache, StorageEvent instead of polling\n**Architecture (12):** GameContext v2 (25 events→0), schema migrations, Smart SW, rooms cleanup\n**Features:** Account (password/export/delete), friends (decline/remove), WS reconnect UI, matchmaking rating, online timers sync\n**Tests:** 41→150 (+109). dist: 14MB→5.8MB (–59%). **Refactor:** Game.jsx –234, Profile.jsx –415 (→979). 10 extracted hooks/components\n\n**Security:**\n- Admin via ENV variable ADMIN_USERNAMES instead of hardcoded name\n- Trust proxy for correct IP behind Nginx (rate limiting)\n- Password validation synced (min 6 chars)\n- Training data size limit (100KB)\n- Graceful shutdown: notify players, close DB\n\n**Performance:**\n- GPU weights: 8.1MB JSON → 3.2MB binary (–60%), lazy load for Hard+ only\n- 6 new SQL indexes (rating_history, achievements, training, seasons, daily, puzzles)\n- Analytics endpoint: LIMIT 2000 instead of loading all games\n- last_seen throttle: update once per 5 min instead of every request\n- N+1 profile query → single SQL with RANK() OVER\n\n**Architecture:**\n- React Context (GameContext) replaces 5 window.* globals\n- PM2: log rotation, graceful shutdown, file logging\n- CI/CD: deploy server/ directory instead of file list\n- Binary weights conversion utility\n\n**New in v4.4.26+:**\n- dist: 14MB→5.8MB (–59%) — removed JSON fallback weights\n- PWA Install Prompt: Install button\n- ProfileAnalytics, ProfileAccount, ProfileFriends — extracted from Profile.jsx\n- BreadcrumbList + FAQPage JSON-LD, preconnect, noscript\n- DB maintenance, SQLite pragmas, anti-cheat, 404 handler\n- CORS preflight cache, rate limiter cleanup, X-Response-Time\n- Online lobby: live games indicator',
  'release', '2026-04-02 22:00:00')
