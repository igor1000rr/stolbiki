...
updatePost('roadmap-2026', 'Планы на 2026', 'Roadmap 2026',
  '✅ Android-приложение\n✅ GPU-нейросеть 840K\n✅ AI Game Review\n✅ Puzzle Rush + Live Arena\n✅ 5 уроков + 33 ачивки\n✅ 11 тем + 17 скинов\n✅ Маскот Снуппи\n✅ SEO path routing + /en/\n✅ Replay sharing\n✅ 24 настройки\n✅ 15 метрик аналитики\n✅ Конфетти + Error Boundary\n✅ Ranked matchmaking\n✅ Online таймеры\n✅ WS reconnect\n✅ Архитектурный аудит (v4.4): 30+ фиксов безопасности и производительности\n✅ GameContext EventEmitter: 25 CustomEvent → чистая архитектура\n✅ GPU weights binary (–60%) + lazy load\n✅ Schema migrations + graceful shutdown + log rotation\n\n✅ Управление аккаунтом + Error reporting + Smart SW\n✅ Тесты: 41→157, dist –59%, 10 extracted модулей\n\nДалее:\n→ Google Play\n→ AI v4 (GPU checkpoint v1493)\n→ iOS\n→ CDN\n→ Game.jsx → custom hooks',
  '✅ Android app\n✅ GPU neural net 840K\n✅ AI Game Review\n✅ Puzzle Rush + Live Arena\n✅ 5 lessons + 33 achievements\n✅ 11 themes + 17 skins\n✅ Mascot Snoopy\n✅ SEO path routing + /en/\n✅ Replay sharing\n✅ 24 settings\n✅ 15 analytics metrics\n✅ Confetti + Error Boundary\n✅ Ranked matchmaking\n✅ Online timers\n✅ WS reconnect\n✅ Architecture audit (v4.4): 30+ security & performance fixes\n✅ GameContext EventEmitter: 25 CustomEvents → clean architecture\n✅ GPU weights binary (–60%) + lazy load\n✅ Schema migrations + graceful shutdown + log rotation\n\n✅ Account management + Error reporting + Smart SW\n✅ Tests: 41→157, dist –59%, 10 extracted modules, 10 extracted модулей\n\nNext:\n→ Google Play\n→ AI v4 (GPU checkpoint v1493)\n→ iOS\n→ CDN\n→ Game.jsx → custom hooks',
...
addPost('v471-security-audit', 'v4.7.1: Аудит безопасности, WS reconnect, AuthContext', 'v4.7.1: Security audit, WS reconnect, AuthContext',
  'Комплексный апдейт: безопасность, стабильность онлайна, рефакторинг клиента.\n\n**🔐 Безопасность**\n**Token revocation** — новое поле `token_version` в users. Админ одной кнопкой может отозвать все токены юзера. При смене пароля (reset_password) token_version инкрементируется автоматически — старые JWT мгновенно невалидны.\n**SQL injection fix** — admin analytics переведены с template literal на параметризованные запросы `datetime(\'now\', ?)`. Закрыт вектор через `?days=` query.\n**Token grace period закрыт** — старые JWT без поля `tv` отклоняются если в БД token_version > 0 (раньше была дыра после миграции).\n**WS maxPayload 16KB → 4KB** — защита от amplification-атак через большие пакеты.\n\n**🔌 Онлайн: WebSocket reconnect**\nНовая команда `reconnect` на сервере. После разрыва связи клиент может восстановить слот в партии за 1 запрос: сервер проверяет JWT, находит слот игрока в комнате, подменяет ws, отменяет таймер удаления комнаты и отправляет снапшот текущего gameState. Партия продолжается без потери состояния.\n\n**🚦 Раздельные WS rate limits**\nДо этого был единый лимит 15 сообщений/сек на всё — спам эмодзи-реакциями мог дропнуть ход. Теперь:\n- Геймплей (move, resign, join, reconnect…) — 20/сек\n- Чат, реакции, draw offer, rematch offer — 5/сек\n\nСпамить эмодзи можно сколько угодно — ходы никогда не дропаются.\n\n**🎨 Новый favicon**\nЗолотые «SH» на синем градиенте. `favicon.ico` теперь multi-size (16/32/48) — раньше был только 16×16.\n\n**🧹 Рефакторинг клиента**\n**AuthContext** — единый провайдер auth-состояния. В App.jsx были два практически одинаковых useEffect со `storage` + `visibilitychange` listener\'ами (один для authUser, другой для isAdmin). Теперь всё в одном хуке `useAuth()`.\n**APP_VERSION auto-inject** — версия читается из package.json в `vite.config.js` через `define: { __APP_VERSION__ }`. Раньше нужно было править в 3 местах (package.json, App.jsx, Changelog).\n**dashboard.json / replays.json → /public** — грузятся через fetch, не бандлятся в JS. Минус ~50KB из основного бандла.\n**Changelog.jsx** переведён на импорт из `src/data/changelog.js` (было 822 строки инлайн в компоненте).\n\n**⚙️ CI/CD**\n**GitHub Actions v5** — checkout, setup-node, setup-java все обновлены до v5. Работают на Node 24, варнинг про депрекейт Node 20 убран.\n**Deploy pipeline** — install server dependencies отдельным шагом (`cd server && npm ci`), версия синхронизируется из корневого package.json в server/package.json при сборке (чтобы /api/health показывал актуальную).\n\n**🧼 Мелочи**\n- Убран бейдж ✓A→B и строка подсказки клавиш в игре (дубль — есть overlay по `?`).\n- Удалён `audit-full.patch` (72KB мусора) из корня репо.\n- `react-chartjs-2` удалён как неиспользуемый.\n- `package.json`: добавлены `engines` — node >=22, npm >=10.\n- `lastSeenCache`: исправлена очистка записей с разными типами значений (number vs {at, tv}).\n- `server/package-lock.json` синхронизирован с package.json (CI падал на `npm ci` после добавления supertest).',
  'Comprehensive update: security, online stability, client refactor.\n\n**🔐 Security**\n**Token revocation** — new `token_version` field in users. Admin can revoke all user tokens with one click. Reset_password auto-increments token_version — old JWTs invalidated instantly.\n**SQL injection fix** — admin analytics moved from template literals to parameterized queries `datetime(\'now\', ?)`. Closed the vector via `?days=` query.\n**Token grace period closed** — old JWTs without `tv` field rejected if DB token_version > 0 (was a hole after migration).\n**WS maxPayload 16KB → 4KB** — protection against amplification attacks via large packets.\n\n**🔌 Online: WebSocket reconnect**\nNew `reconnect` command on server. After disconnect, client can restore its slot in 1 request: server verifies JWT, finds player slot in room, swaps ws, cancels room delete timer, sends current gameState snapshot. Game continues without state loss.\n\n**🚦 Split WS rate limits**\nPreviously a single limit of 15 msg/sec for everything — emoji reaction spam could drop a move. Now:\n- Gameplay (move, resign, join, reconnect…) — 20/sec\n- Chat, reactions, draw offer, rematch offer — 5/sec\n\nEmoji spam all you want — moves never drop.\n\n**🎨 New favicon**\nGolden "SH" on blue gradient. `favicon.ico` is now multi-size (16/32/48) — previously only 16×16.\n\n**🧹 Client refactor**\n**AuthContext** — single provider for auth state. App.jsx had two nearly-identical useEffects with `storage` + `visibilitychange` listeners (one for authUser, another for isAdmin). Now it\'s one `useAuth()` hook.\n**APP_VERSION auto-inject** — version is read from package.json in `vite.config.js` via `define: { __APP_VERSION__ }`. Previously had to be edited in 3 places.\n**dashboard.json / replays.json → /public** — loaded via fetch, not bundled in JS. ~50KB off main bundle.\n**Changelog.jsx** switched to import from `src/data/changelog.js` (was 822 inline lines in the component).\n\n**⚙️ CI/CD**\n**GitHub Actions v5** — checkout, setup-node, setup-java all bumped to v5. Running on Node 24, Node 20 deprecation warning gone.\n**Deploy pipeline** — install server dependencies as separate step (`cd server && npm ci`), version synced from root package.json to server/package.json on build (so /api/health shows the current one).\n\n**🧼 Cleanup**\n- Removed ✓A→B badge and keyboard hint line in game (duplicate — there\'s a `?` overlay).\n- Removed `audit-full.patch` (72KB of junk) from repo root.\n- Removed unused `react-chartjs-2`.\n- `package.json`: added `engines` — node >=22, npm >=10.\n- `lastSeenCache`: fixed cleanup of entries with mixed value types (number vs {at, tv}).\n- `server/package-lock.json` synced with package.json (CI was failing on `npm ci` after adding supertest).',
  'release', '2026-04-06 12:00:00')

// Pin только v4.7.1
db.prepare("UPDATE blog_posts SET pinned=0").run()
db.prepare("UPDATE blog_posts SET pinned=1 WHERE slug='v471-security-audit'").run()
