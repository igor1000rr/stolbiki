...
addPost('v43-seo', 'v4.3: SEO path routing, replay sharing, 6 анимаций', 'v4.3: SEO path routing, replay sharing, 6 animations',
  'Большое обновление с фокусом на SEO и визуал:\n\n**SEO: path routing** — /#game → /game, /#rules → /rules. Теперь каждая страница индексируется Google и Яндекс отдельно. Было 2 страницы, стало 16.\n**/en/ URL routing** — переключение языка меняет URL (/en/game). hreflang теги, sitemap с 16 страницами.\n**Replay sharing** — сохраняйте партии и делитесь ссылкой. Кнопка «🔗 Поделиться» в результатах.\n**Конфетти** — canvas-анимация из 120 частиц при победе.\n**Лендинг** — 6 тем с анимацией падающих блоков, 6 карточек фич с живыми SVG-анимациями (эквалайзер, летящий пакет, прогресс-бар).\n**Error Boundary** — JS-краш → Снуппи + перезагрузка.\n**Звуки** — 7 звуковых эффектов подключены (были написаны, но не вызывались).\n**Валидация** — server/validate.js, XSS защита.\n**Автобэкап** — SQLite каждые 6 часов.',
  'Major update focused on SEO and visuals:\n\n**SEO: path routing** — /#game → /game, /#rules → /rules. Each page now indexed by Google separately. Was 2 pages, now 16.\n**/en/ URL routing** — language switch updates URL (/en/game). hreflang tags, sitemap with 16 pages.\n**Replay sharing** — save games and share links. "🔗 Share" button in results.\n**Confetti** — canvas animation of 120 particles on victory.\n**Landing** — 6 themes with falling block animation, 6 feature cards with live SVG animations (equalizer, flying packet, progress bar).\n**Error Boundary** — JS crash → Snoopy + reload.\n**Sounds** — 7 sound effects hooked up (were written but never called).\n**Validation** — server/validate.js, XSS protection.\n**Auto backup** — SQLite every 6 hours.',
