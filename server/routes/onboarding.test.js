...
describe('POST /api/onboarding/complete', () => {
  ...
  it('повторный вызов → 409 без побочных эффектов (идемпотентность)', async () => {
    const { token } = await makeUser()
    const first = await request(app).post('/api/onboarding/complete').set('Authorization', `Bearer ${token}`).send({})
    expect(first.status).toBe(200)
    const balanceAfterFirst = first.body.bricks
    ...
    // Баланс не изменился после повторного (status покажет текущий баланс)
