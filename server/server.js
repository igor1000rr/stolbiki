...
addPost('v35-gpu-neural', 'v3.5: GPU-нейросеть, экстрим, спектатор, рематч', 'v3.5: GPU neural network, extreme, spectator, rematch',
  'Самое масштабное обновление AI и онлайн-режима.\n\n' +
  '**GPU-нейросеть в браузере**\nResNet с 840,321 параметрами загружается прямо в браузер (6.8MB gzip ~1MB). Evaluator гибридный: 60% нейросеть + 40% тактические эвристики (закрытие, золотая, контроль верха). Это первая настольная игра с полноценной AlphaZero-архитектурой прямо в WebView.\n\n' +
  '**Сложность «Экстрим»**\n1500 GPU-симуляций MCTS, ~2 секунды на ход. Первые 1-2 хода используют CPU fallback пока GPU-веса загружаются.\n\n' +
  '**Спектатор-режим**\nНаблюдение за чужими онлайн-партиями в реальном времени. Доска обновляется мгновенно через WebSocket.\n\n' +
  '**Рематч онлайн**\nПосле партии — кнопка «Рематч». Сервер меняет стороны. Оба игрока должны согласиться.\n\n' +
  '**Публичные профили**\nКлик по нику в лидерборде или истории открывает карточку игрока с рейтингом, ачивками и статистикой.\n\n' +
  '**200 головоломок**\nБанк расширен с 50 до 200. 10 шаблонов генерации, ISO-недели для weekly challenge.\n\n' +
  '**Серверная валидация**\nСервер проверяет каждый ход через собственный движок. Защита от эксплойтов.',
  'The biggest AI and online update.\n\n' +
  '**GPU neural network in browser**\nResNet with 840,321 parameters loads directly in the browser (6.8MB gzip ~1MB). Hybrid evaluator: 60% neural net + 40% tactical heuristics. This is the first board game with a full AlphaZero architecture running in WebView.\n\n' +
  '**Extreme difficulty**\n1500 GPU MCTS simulations, ~2 seconds per move. First 1-2 moves use CPU fallback while GPU weights load.\n\n' +
  '**Spectator mode**\nWatch other players\' online games in real-time. Board updates instantly via WebSocket.\n\n' +
  '**Online rematch**\nAfter a game — "Rematch" button. Server swaps sides. Both players must agree.\n\n' +
  '**Public profiles**\nClick any nickname in leaderboard or history to see their stats, rating, and achievements.\n\n' +
  '**200 puzzles**\nBank expanded from 50 to 200. 10 generation templates, ISO weeks for weekly challenge.\n\n' +
  '**Server validation**\nServer verifies every move through its own engine. Protection against exploits.',
  'release')
