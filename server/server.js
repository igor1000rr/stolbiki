...
app.get('/api/health', (req, res) => {
  ...
  res.json({
    status: 'ok', version: APP_VERSION, node: process.version, uptime: Math.round(process.uptime()),
    users: db.prepare('SELECT COUNT(*) as c FROM users').get().c,
    rooms: rooms.size,
    activeRooms: [...rooms.values()].filter(r => r.state === 'playing' && r.players.length === 2).length,
    matchQueue: matchQueue.length, memoryMB: Math.round(mem.heapUsed / 1024 / 1024),
...
app.post('/api/track', rateLimit(60000, 300), (req, res) => {
  try {
    ...
    let metaJson = '{}'
    try { if (meta && typeof meta === 'object') metaJson = JSON.stringify(meta).slice(0, 500) } catch {}
    let userId = null
    const authHeader = req.headers.authorization
    if (authHeader) { try { userId = jwt.verify(authHeader.replace('Bearer ', ''), JWT_SECRET).id } catch {} }
...
app.post('/api/error-report', rateLimit(60000, 5), (req, res) => {
  ...
  let userId = null
  try { const authHeader = req.headers.authorization; if (authHeader) { const decoded = jwt.verify(authHeader.replace('Bearer ', ''), JWT_SECRET); userId = decoded.id } } catch {}
...
if (!isTest) {
  ...
  const gracefulShutdown = (signal) => {
    ...
    server.close(() => { console.log('✅ HTTP/WS сервер закрыт'); db.close(); console.log('✅ SQLite закрыт'); process.exit(0) })
