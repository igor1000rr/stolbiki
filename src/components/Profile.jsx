  ...
  return (
    <div>
      ...
      {tab === 'analytics' && (
        <div>
          {!analyticsData ? (
            <div className="dash-card" style={{ textAlign: 'center', padding: 40, color: 'var(--ink3)' }}>{en ? 'Loading...' : 'Загрузка...'}</div>
          ) : analyticsData.empty ? (
            <div className="dash-card" style={{ textAlign: 'center', padding: 40, color: 'var(--ink3)' }}>{en ? 'Play some games first!' : 'Сначала сыграйте несколько партий!'}</div>
          ) : (<>
            {/* Основные числа */}
