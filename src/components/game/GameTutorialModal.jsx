/**
 * Модалка «Как играть» — показывается при первом заходе в игру.
 * Флаг в localStorage: stolbiki_tutorial_seen.
 *
 * Вынесено из Game.jsx.
 */
export default function GameTutorialModal({ lang, isNative, onDismiss }) {
  const en = lang === 'en'
  return (
    <div
      style={{
        ...
        padding: isNative ? 12 : 20, overflowY: 'auto',
      }}
      onClick={onDismiss}
    >
      <div
        style={{
          maxWidth: 420, width: '100%', background: 'var(--surface)', borderRadius: 16,
          padding: isNative ? '20px 16px' : '28px 24px',
          border: '1px solid var(--surface3)', boxShadow: '0 20px 60px rgba(0,0,0,0.5)', margin: 'auto',
        }}
