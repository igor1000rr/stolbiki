...
const VERSIONS = [
  {
    version: '4.4',
    date: '2026-04-02',
    title_ru: 'Архитектурный аудит — 53 коммита, 157 тестов, dist –59%',
    title_en: 'Architecture audit — 53 commits, 157 tests, dist –59%',
    changes_ru: [
      ...
      { type: 'new', text: 'Таб «Аккаунт»: смена пароля, экспорт JSON (GDPR), удаление аккаунта' },
      { type: 'new', text: 'Друзья: decline + remove endpoints + UI кнопки' },
      { type: 'new', text: 'WS Reconnect UI: прогресс-бар попытки/макс, задержка' },
      { type: 'new', text: 'Matchmaking: рейтинг и ELO ±200 при поиске' },
      { type: 'new', text: 'Online timers: серверная синхронизация при каждом ходе + timeUp' },
      { type: 'new', text: 'Error reporting: ErrorBoundary + global handlers → server DB + admin endpoint' },
      { type: 'new', text: 'Schema migrations: schema_version + runMigration() (3 миграции)' },
      { type: 'new', text: 'Smart Service Worker: cache-first assets, stale-while-revalidate images, network-first navigation' },
      { type: 'new', text: 'Тесты: 41 → 85. validate (31), helpers (7), стресс-тест 500 партий (6)' },
      { type: 'new', text: 'Health endpoint: версия, память, matchQueue, schemaVersion' },
      { type: 'fix', text: 'Безопасность: Admin ENV, trust proxy, JWT 7d+refresh, replay limits, training data limit' },
      { type: 'fix', text: 'Auth: 2×setInterval(2s) → StorageEvent + visibilitychange' },
      ...
      { type: 'fix', text: '6 SQL индексов, N+1→RANK() OVER, analytics LIMIT 2000' },
      ...
      { type: 'fix', text: 'Game.jsx: 1723→1489. Extracted: GameResultPanel, useSessionStats + DailyChallenge из Online.jsx' },
      ...
      { type: 'new', text: 'Тесты: 41→157 (+116). validate, helpers, stress, engine, score validation' },
      ...
      { type: 'fix', text: 'CORS preflight cache (1ч), rate limiter cleanup, SQLite pragmas (busy_timeout, synchronous)' },
      { type: 'new', text: 'SEO: BreadcrumbList + FAQPage JSON-LD, preconnect, noscript fallback, Telegram meta' },
      { type: 'new', text: 'DB maintenance: ежедневная очистка error_reports, WAL checkpoint' },
      { type: 'fix', text: 'Anti-cheat: минимум 8 ходов для победы. 404 handler. X-Response-Time header' },
      { type: 'new', text: '.env.example + README.md полная документация' },
    ],
    changes_en: [
      { type: 'new', text: 'GameContext v2: multi-listener EventEmitter — 25 CustomEvents → 0' },
      { type: 'new', text: 'GPU weights: 8.1MB JSON → 3.2MB binary (–60%), lazy load for Hard+' },
      { type: 'new', text: 'Account tab: change password, export JSON (GDPR), delete account' },
      { type: 'new', text: 'Friends: decline + remove endpoints + UI buttons' },
      { type: 'new', text: 'WS Reconnect UI: progress bar with attempt/max, delay' },
      { type: 'new', text: 'Matchmaking: rating and ELO ±200 range during search' },
      { type: 'new', text: 'Online timers: server sync on every move + timeUp handler' },
      { type: 'new', text: 'Error reporting: ErrorBoundary + global handlers → server DB + admin endpoint' },
      { type: 'new', text: 'Schema migrations: schema_version + runMigration() (3 migrations)' },
      { type: 'new', text: 'Smart Service Worker: cache-first assets, stale-while-revalidate images, network-first navigation' },
      { type: 'new', text: 'Tests: 41 → 85. validate (31), helpers (7), stress test 500 games (6)' },
      { type: 'new', text: 'Health endpoint: version, memory, matchQueue, schemaVersion' },
      { type: 'fix', text: 'Security: Admin ENV, trust proxy, JWT 7d+refresh, replay limits, training data limit' },
      { type: 'fix', text: 'Auth: 2×setInterval(2s) → StorageEvent + visibilitychange' },
      ...
      { type: 'fix', text: '6 SQL indexes, N+1→RANK() OVER, analytics LIMIT 2000' },
      { type: 'fix', text: 'Board.jsx: useEffect deps → stable key, rooms/matchQueue cleanup' },
      { type: 'fix', text: 'Cache-Control: 6 endpoints (blog, stats, leaderboard, rush, content)' },
      { type: 'fix', text: 'Graceful shutdown, PM2 logs, CI/CD server/ directory, backup sqlite3' },
      { type: 'fix', text: 'Game.jsx: 1723→1489. Extracted: GameResultPanel, useSessionStats + DailyChallenge из Online.jsx' },
      ...
      { type: 'new', text: 'Tests: 41→157 (+116). validate, helpers, stress, engine, score validation' },
      ...
      { type: 'fix', text: 'CORS preflight cache (1h), rate limiter cleanup, SQLite pragmas (busy_timeout, synchronous)' },
      { type: 'new', text: 'SEO: BreadcrumbList + FAQPage JSON-LD, preconnect, noscript fallback, Telegram meta' },
      { type: 'new', text: 'DB maintenance: daily error_reports cleanup, WAL checkpoint' },
      { type: 'fix', text: 'Anti-cheat: min 8 turns to win. 404 handler. X-Response-Time header' },
      { type: 'new', text: '.env.example + README.md full documentation' },
    ],
  },
  {
    version: '4.4',
