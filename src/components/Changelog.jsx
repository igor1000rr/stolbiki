...
const VERSIONS = [
  {
    ...
    changes_ru: [
      { type: 'new', text: 'Тесты: 41 → 157 (+116). Движок, валидация, стресс, edge cases, score' },
      { type: 'new', text: 'dist: 14MB → 5.8MB (–59%) — GPU weights только binary, убран JSON fallback' },
      { type: 'new', text: 'GameContext v2: 25 CustomEvent → 0, 5 window globals → 0, 4 polling → 0' },
      ...
      { type: 'new', text: 'WS Reconnect UI + Matchmaking рейтинг + Online timers sync' },
      { type: 'new', text: 'Error reporting: ErrorBoundary + global handlers → server DB + admin' },
      { type: 'new', text: 'Smart Service Worker: 4 стратегии кеширования, BUILD_HASH версионирование' },
      { type: 'new', text: 'Schema migrations: versioned (3), graceful shutdown, DB maintenance' },
      { type: 'new', text: 'Health endpoint: версия, память, rooms, matchQueue, API stats, rateLimits' },
      { type: 'new', text: 'SEO: BreadcrumbList + FAQPage JSON-LD, preconnect, noscript, hreflang' },
      ...
      { type: 'fix', text: 'Game.jsx: 1723 → 1489 (–234). Extracted: GameResultPanel, useSessionStats' },
      ...
      { type: 'fix', text: 'Безопасность: Admin ENV, trust proxy, JWT 7d+refresh, anti-cheat, Permissions-Policy' },
      { type: 'fix', text: 'GPU weights: 8.1MB → 3.2MB binary (–60%), lazy load для Hard+' },
      { type: 'fix', text: '6 SQL индексов, N+1→RANK(), analytics LIMIT, last_seen throttle' },
      { type: 'fix', text: 'Auth: polling → StorageEvent, applySettings batch, collector кеш' },
      { type: 'fix', text: 'CI/CD: test → build → backup DB → deploy. npm cache, CORS preflight 1ч' },
      { type: 'fix', text: 'Cache-Control: 7 endpoints. ETag, X-Response-Time, 404 handler' },
      { type: 'fix', text: 'Nginx: gzip, Cache-Control split (HTML no-store, assets immutable)' },
    ],
    changes_en: [
      { type: 'new', text: 'Tests: 41 → 157 (+116). Engine, validation, stress, edge cases, score' },
      { type: 'new', text: 'dist: 14MB → 5.8MB (–59%) — GPU weights binary only, removed JSON fallback' },
      { type: 'new', text: 'GameContext v2: 25 CustomEvents → 0, 5 window globals → 0, 4 polling → 0' },
      ...
      { type: 'new', text: 'WS Reconnect UI + Matchmaking rating + Online timers sync' },
      { type: 'new', text: 'Error reporting: ErrorBoundary + global handlers → server DB + admin' },
      { type: 'new', text: 'Smart Service Worker: 4 caching strategies, BUILD_HASH versioning' },
      { type: 'new', text: 'Schema migrations: versioned (3), graceful shutdown, DB maintenance' },
      { type: 'new', text: 'Health endpoint: version, memory, rooms, matchQueue, API stats, rateLimits' },
      { type: 'new', text: 'SEO: BreadcrumbList + FAQPage JSON-LD, preconnect, noscript, hreflang' },
      ...
      { type: 'fix', text: 'Profile.jsx: 1394 → 979 (–415). Extracted: ProfileAccount, ProfileFriends, ProfileAnalytics' },
      { type: 'fix', text: 'Online.jsx: 696 → 614 (–82). Extracted: DailyChallenge' },
      { type: 'fix', text: 'Security: Admin ENV, trust proxy, JWT 7d+refresh, anti-cheat, Permissions-Policy' },
      { type: 'fix', text: 'GPU weights: 8.1MB → 3.2MB binary (–60%), lazy load for Hard+' },
      { type: 'fix', text: '6 SQL indexes, N+1→RANK(), analytics LIMIT, last_seen throttle' },
      { type: 'fix', text: 'Auth: polling → StorageEvent, applySettings batch, collector cache' },
      { type: 'fix', text: 'CI/CD: test → build → backup DB → deploy. npm cache, CORS preflight 1h' },
      { type: 'fix', text: 'Cache-Control: 7 endpoints. ETag, X-Response-Time, 404 handler' },
      { type: 'fix', text: 'Nginx: gzip, Cache-Control split (HTML no-store, assets immutable)' },
    ],
  },
