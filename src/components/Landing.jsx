        ...
        <div className={`l-feat-grid ${featVis ? 'in' : ''}`} ref={featRef}>
          {[
            { color: '#4a9eff', title: en ? 'AlphaZero AI' : 'AI нейросеть', desc: en ? 'Trained on 239K games via self-play. 4 difficulty levels.' : 'Обучена на 239K партиях. 4 уровня сложности.',
              visual: <svg viewBox="0 0 120 56" className="l-feat-svg"><g className="l-feat-bars">{[18,32,24,40,28,36,20,44,30,22].map((h,i)=><rect key={i} x={4+i*12} y={48-h} width={8} height={h} rx={2} fill={i%2===0?'#4a9eff':'#ff6066'} opacity={0.7} style={{animationDelay:`${i*0.08}s`}}/>)}</g><text x="60" y="10" textAnchor="middle" fill="#ffc145" fontSize="10" fontWeight="700" className="l-feat-pulse" style={{filter:'drop-shadow(0 0 4px #ffc14580)'}}>★ 98% win rate</text></svg> },
            ...
            { color: '#ff6066', title: en ? 'Arena & Progression' : 'Арена и прогресс', desc: en ? 'Tournaments, daily missions, 33 achievements, levels.' : 'Турниры, миссии, 33 ачивки, уровни.',
              visual: <svg viewBox="0 0 120 56" className="l-feat-svg"><g className="l-feat-bars">{[12,20,32,24,40,16,28,36].map((h,i)=><rect key={i} x={8+i*14} y={48-h} width={10} height={h} rx={2} fill={`hsl(${340+i*8},70%,60%)`} opacity={0.6} style={{animationDelay:`${i*0.06}s`}}/>)}</g><text x="60" y="10" textAnchor="middle" fill="#ffc145" fontSize="10" fontWeight="700" className="l-feat-pulse" style={{filter:'drop-shadow(0 0 4px #ffc14580)'}}>🏆 XP +50</text></svg> },
          ].map((f, i) => (
