/**
 * Golden Rush — движок для 4-игрового режима (FFA и 2v2).
 *
 * НЕ дублирует и НЕ заменяет базовый server/game-engine.js — тот остаётся для
 * 2-игровой игры с AlphaZero-AI v7, его трогать нельзя.
 *
 * Спека режима: docs/modes/golden-rush.md
 *
 * Топология: 9 стоек крестом. Индексация:
 *   0 — центр (golden)
 *   1,2 — игрок 0 (close, far)
 *   3,4 — игрок 1
 *   5,6 — игрок 2
 *   7,8 — игрок 3
 *
 * close (order=1) — ближняя к игроку, far (order=2) — дальняя (к центру).
 *
 * Правила закрытия:
 *  – order=1 → закрывается при 11 блоках (flag = цвет верхней группы)
 *  – order=2 → только если paired order=1 уже закрыта; до этого cap = 10
 *  – center → только если есть eligible игрок (замкнул обе свои стойки);
 *    до этого cap = 10. Flag центра = цвет первого в FIFO-очереди eligible.
 *
 * Скоринг:
 *  – +1 за каждый блок своего цвета на доске
 *  – +5 за flag на order=1, +8 за order=2
 *  – +15 за flag на центре
 *  – 2v2: +5 каждому если оба в команде замкнули свои линии (=+10 на команду)
 */
...
// Игроки сидят: 0=низ, 1=лево, 2=верх, 3=право (по часовой)
export const STAND_COORDS = [
  { x: 0,     y: 0    },  // 0 center
  { x: 0,     y: 1    },  // 1 p0 close
  { x: 0,     y: 0.45 },  // 2 p0 far
  { x: -1,    y: 0    },  // 3 p1 close
...
export const PLAYER_COLORS = ['#4a9eff', '#ff6b6b', '#3dd68c', '#ffc145']
export const PLAYER_NAMES_RU = ['Синий', 'Красный', 'Зелёный', 'Жёлтый']
export const PLAYER_NAMES_EN = ['Blue', 'Red', 'Green', 'Yellow']
